...
namespace VendingMachineTests
{
    public class VendingMachineUnitTests
    {
        ...
        [Fact]
        public void InsertCoin_IncreasesBalance()
        {
        ...
        [Fact]
        public void SelectProduct_WithEnoughBalance()
        {
            var vm = new VendingMachine();
            vm.InsertCoin(100); 
        ...
        [Fact]
        public void SelectProduct_WithoutEnoughBalance_DoesNotChangeQuantity()
        {
            var vm = new VendingMachine();
        ...
        // Тест 4: при недостаточном балансе сумма собранных денег не уменьшается
        [Fact]
        public void SelectProduct_InsufficientBalance_DoesNotReduceCollectedMoney()
        {
            var vm = new VendingMachine();
            int initialCollected = vm.GetCollectedMoney();
            vm.SelectProduct(1); // попытка покупки без денег
            Assert.Equal(initialCollected, vm.GetCollectedMoney()); // собранная сумма не изменилась
        }

        // Тест 5: отмена операции обнуляет баланс
        [Fact]
        public void CancelOperation_ReturnsBalanceToZero()
        {
            var vm = new VendingMachine();
            vm.InsertCoin(30);
        ...
        // Тест 6: администратор добавляет товар, количество увеличивается
        [Fact]
        public void Admin_AddProduct_IncreasesQuantity()
        {
            var vm = new VendingMachine();
            int initialQty = vm.GetProductQuantity(1);
        ...
        // Тест 7: администратор собирает деньги, собранная сумма сбрасывается
        [Fact]
        public void Admin_CollectMoney_ResetsCollectedMoney()
        {
            var vm = new VendingMachine();
            vm.InsertCoin(50);
            vm.SelectProduct(1); 
        ...
        // Тест 8: попытка купить товар с количеством 0 не разрешается
        [Fact]
        public void SelectProduct_WhenQuantityZero_DoesNotAllowPurchase()
        {
            var vm = new VendingMachine();
            int index = 2;
        ...
        // Тест 9: вставка нескольких монет суммируется корректно
        [Fact]
        public void InsertMultipleCoins_SumsBalanceCorrectly()
        {
            var vm = new VendingMachine();
            vm.InsertCoin(10);
            vm.InsertCoin(20);
            vm.InsertCoin(5);
            Assert.Equal(35, vm.GetBalance()); // проверяем итоговый баланс
        }

        // Тест 10: покупка товара с остатком денег (сдачей)
        [Fact]
        public void SelectProduct_PurchaseWithChange_BalanceIsCorrect()
        {
            var vm = new VendingMachine();
